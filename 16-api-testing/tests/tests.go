...
import (
	...
	"github.com/jacky-htg/go-services/services/database/databasetest"
...
func NewUnit(t *testing.T) (*sqlx.DB, func()) {
	...
	db, err := sqlx.Open(os.Getenv("DB_DRIVER"), "root:1234@tcp(localhost:33060)/rebel_db?parseTime=true")
	if err != nil {
		t.Fatalf("opening database connection: %v", err)
	}

	dbo, err := sql.Open(os.Getenv("DB_DRIVER"), "root:1234@tcp(localhost:33060)/rebel_db?parseTime=true")
	if err != nil {
		t.Fatalf("opening database connection: %v", err)
	}
	...
	for attempts := 1; attempts <= maxAttempts; attempts++ {
		...
		time.Sleep(time.Duration(attempts) * 10000 * time.Millisecond)
	}
