...
func (u *Checks) Health(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	var health struct {
		Status string `json:"status"`
	}

	// Check if the database is ready.
	if err := database.StatusCheck(ctx, u.Db); err != nil {
	...
	health.Status = "ok"
	return api.ResponseOK(ctx, w, health, http.StatusOK)
}
